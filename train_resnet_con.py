...
from dataset.CNCSP import Dataset
...
def train_resnet(args):
    ...
    train_dataset = Dataset(data_dict=data_dict, data_idxs=args.train_names, is_train=True, get_name=True, class_weights=args.sample_weights)
    val_dataset = Dataset(data_dict=data_dict, data_idxs=args.val_names, is_train=False, get_name=True)
        ...
        for data in train_loader:
            ...
            if iterations % 400 == 0:
                ...
                if mean_loss < best_loss:
                    best_loss = mean_loss
                    save_file = os.path.join(args.save_model, "resnet50.pth".format(args.log_time))
    ...
    save_file = os.path.join(args.save_model, "last.pth".format(args.log_time))
    args.resnet_model = save_file
    torch.save({"model": model.state_dict(),
                "optim": optimizer.state_dict()}, save_file)
